...
(function(g){
    ...
    return function(){
        try{
            var e=a.responseXML;
            if(e&&e.firstChild)return c(a)
                }catch(d){}(e=g.parseXML(a.responseText))&&e.firstChild?(a=g.extend({},a,{
            responseXML:e
...
function(a){
    ...
    if(a){
        ...
        for(var e=0;e<a.length;e++){
            var d=a[e].type,h=a[e].file;
            d||(d=f.extType(g.extension(h)),a[e].type=d);
            if((h=g.extensionmap.types[d])&&jwplayer.vid.canPlayType(h))b||(b=d),d==b&&c.push(g.extend({},a[e]))
                }
            }
        return c
};
...
g.ajax=function(a,b,c){
    ...
    if(e&&g.exists(d.XDomainRequest))k=new XDomainRequest,k.onload=f(k,a,b,c),k.onerror=
        h(c,a,k);
    else if(g.exists(d.XMLHttpRequest)){
        var m=k=new XMLHttpRequest,l=a;
        k.onreadystatechange=function(){
            if(4===m.readyState)switch(m.status){
                case 200:
                    f(m,l,b,c)();
                    break;
                case 404:
...
g.bounds=function(a){
    ...
    var b=a,c=0,f=0,e=isNaN(a.offsetWidth)?0:a.offsetWidth;
    a=isNaN(a.offsetHeight)?0:a.offsetHeight;
    do c+=isNaN(b.offsetLeft)?0:b.offsetLeft,f+=isNaN(b.offsetTop)?0:b.offsetTop;while(b=b.offsetParent);
    return{
        left:c,
        top:f,
        width:e,
...
function d(a){
    ...
    for(var m in q){
        var j;
            ...
            j=m;
            var p=q[m];
            if("undefined"===typeof p)j=void 0;
            else{
                var z=n?" !important":"";
                if(isNaN(p))j=p.match(/png|gif|jpe?g/i)&&0>p.indexOf("url")?"url("+p+")":p+z;else switch(j){
...
j.unblock=function(){
    k=Math.max(k-1,0);
    if(0==k)for(var b in a)f(b)
        };

g.clearCss=function(b){
    for(var e in c)0<=e.indexOf(b)&&delete c[e];for(var f in a)0<=f.indexOf(b)&&(a[f].innerHTML="")
        };
        ...
        if(d&&(f||(f=h.UNIFORM),a&&b&&c&&k)){
            ...
            switch(f.toLowerCase()){
                ...
                default:
                    ...
                    "jwexactfit"):(c*=m,k*=m):0.95<k*e/b?(j=!0,r="jwexactfit"):(c*=e,k*=e),j&&(m=Math.ceil(100*b/k)/100,e=Math.ceil(100*a/c)/100)
                    }
                    l?j?(d.style.width=c+"px",d.style.height=k+"px",g.scale(d,e,m,(a-c)/2/e,(b-k)/2/m)):(d.style.width="",d.style.height=""):(d.className=d.className.replace(/\s*jw(none|exactfit|uniform|fill)/g,""),d.className+=" "+r)
            }
        };
    ...
    d.onreadystatechange=function(){
        if(4===d.readyState)if(200===d.status){
            for(var c=d.responseText,k=[{
                begin:0,
                text:""
            }],c=c.replace(/^\s+/,"").replace(/\s+$/,""),c=c.split("\x3c/p\x3e"),e=[],m=0;m<c.length;m++)0<=c[m].indexOf("\x3cp")&&(c[m]=c[m].substr(c[m].indexOf("\x3cp")+2).replace(/^\s+/,"").replace(/\s+$/,
                ""),e.push(c[m]));
            c=e;
            ...
            }else k=d.status,0==k?f("Crossdomain loading denied: "+a):404==k?f("DFXP File not found: "+a):f("Error "+k+" loading DFXP file: "+
            a)
        }
        }
...
(function(g){
    ...
    (h.jwparser=function(){}).parseEntry=function(f,d){
...
(function(g){
    var h=jwplayer.utils,f=h.xmlAttribute,d=g.localName,a=g.textContent,b=g.numChildren,c=g.mediaparser=function(){};

    c.parseGroup=function(k,e){
        var m,l,j=[];
        for(l=0;l<b(k);l++)if(m=k.childNodes[l],"media"==m.prefix&&d(m))switch(d(m).toLowerCase()){
            case "content":
        ...
        for(l=0;l<j.length;l++)e.tracks.push(j[l]);
        return e
        }
    })(jwplayer.html5.parsers);
(function(g){
    ...
    var f=
jwplayer.utils,d=g.textContent,a=g.getChildNode,b=g.numChildren,c=g.localName;
g.rssparser={};

g.rssparser.parse=function(f){
    for(var e=[],d=0;d<b(f);d++){
        var l=a(f,d);
        if("channel"==c(l).toLowerCase())for(var g=0;g<b(l);g++){
            var r=a(l,g);
        ...
        function a(a){
            ...
            for(a=0;a<c.length;a++)if("WEBVTT"!=c[a]){
                ...
                try{
                    ...
                    if(t[n+1]){
                        e.text=t[n+1];
                        for(n+=2;n<t.length;n++)e.text+="\x3cbr/\x3e"+t[n]
                            }
                        }catch(p){}
...
(function(g){
    var h=jwplayer.utils,f=jwplayer.events,d=f.state,a=h.css,b="playing",c=document;
    g.captions=function(a,e){
        function m(a){
            ...
            function q(){
            (new jwplayer.html5.parsers.srt(n,
                m)).load(B)
            }
            function n(a){
            v.populate(a);
            ...
            for(var b=0;b<A.length;b++)a.push({
                label:A[b].label
                });
            return a
            }
...
w.jwAddEventListener(f.JWPLAYER_PLAYLIST_ITEM,function(){
    ...
    A=[];
    v.update(0);
    for(var a=w.jwGetPlaylist()[w.jwGetPlaylistIndex()].tracks,b=[],c=0,e="",d=0,e="",c=0;c<a.length;c++)e=
        a[c].kind.toLowerCase(),("captions"==e||"subtitles"==e)&&b.push(a[c]);
    for(c=E=0;c<b.length;c++)if(e=b[c].file)b[c].label||(b[c].label=c.toString()),A.push(b[c]);for(c=0;c<A.length;c++)if(A[c]["default"]){
        d=c+1;
        break
    }
    if(e=h.getCookies().captionLabel){
        a=z();
        for(c=0;c<a.length;c++)if(e==a[c].label){
            d=c;
            break
        }
        }
...
w.jwAddEventListener(f.JWPLAYER_MEDIA_ERROR,m);
    ...
    w.jwAddEventListener(f.JWPLAYER_READY,
    function(){
        for(var a in u)e&&void 0!=e[a.toLowerCase()]?"color"==a?x.color="#"+String(e.color).substr(-6):x[a]=e[a.toLowerCase()]:x[a]=u[a];v=new jwplayer.html5.captions.renderer(x,s);
        t(!1)
        });
...
(function(g){
    g.captions.renderer=function(h,f){
        function d(b){
            ...
            var a=Math.round(h.fontSize*Math.pow(q/400,0.6)),b=Math.round(1.4*a);
            c({
                maxWidth:q+"px",
                fontSize:a+"px",
                lineHeight:b+"px",
            ...
            function b(){
            for(var a=-1,b=0;b<e.length;b++)if(e[b].begin<=j&&(b==
...
(function(g){
    var h=g.html5,f=g.utils,d=g.events,a=d.state,b=f.css;
    g=f.transitionStyle;
    var c="button",k="text",e="divider",m="slider",l="100%",j={
        display:"none"
    },r={
        display:y
    },q=!1,n=!0,t=null,y=void 0,p=window,z=document;
    h.controlbar=function(g,s){
        ...
        switch(a.type){
            case e:
                var d;
                a.width?(d=J(),d.className="jwblankDivider",b(d,{
                width:parseInt(a.width)
                })):a.element?d=F(a.element):(d=F(a.name),d||(d=J(),d.className="jwblankDivider"));
            ...
            case k:
                ...
                return a;
            case c:
                if("blank"!=a.name)return a=a.name,H(a+"Button").src?(d=J(),d.className="jw"+a+" jwbuttoncontainer",f=z.createElement("button"),f.addEventListener("click",U(a),q),f.innerHTML="\x26nbsp;",d.appendChild(f),
                f=H(a+"Button"),g=H(a+"ButtonOver"),M(B(".jw"+a+" button"),f,g),(f=bb[a])&&M(B(".jw"+a+".jwtoggle button"),H(f+"Button"),H(f+"ButtonOver")),a=K[a]=d):a=t,a;
            break;
            case m:
                f=a.name;
                a=J();
                var n=f+("time"==f?"Slider":"")+"Cap",j=(g="volume"==f)?"Top":"Left",E=g?"Bottom":"Right",s=F(n+j,t,q,q,g),r=F(n+E,t,q,q,g);
                d=J();
                var u=["Rail","Buffer","Progress"],A;
                d.className="jwrail jwsmooth";
                for(var p=0;p<u.length;p++){
                var v="time"==f?"Slider":"",w=f+v+u[p],x=F(w,t,!g,"volume"==f),D=F(w+"Cap"+j,t,q,q,g),I=F(w+
                    "Cap"+E,t,q,q,g),sa=H(w+"Cap"+j),C=H(w+"Cap"+E);
                if(x){
                    var O=J();
                    O.className="jwrailgroup "+u[p];
                    D&&O.appendChild(D);
                    O.appendChild(x);
                    I&&(O.appendChild(I),I.className+=" jwcap"+(g?"Bottom":"Right"));
                    b(B(".jwrailgroup."+u[p]),{
            ...
            if(v=F(f+v+"Thumb",t,q,q,g))b(B("."+v.className),

            {
                opacity:"time"==f?0:1,
                "margin-top":g?v.skin.height/-2:y
                }),v.className+=" jwthumb",(g&&A?A:d).appendChild(v);
            ...
            s&&a.appendChild(s);
            a.appendChild(d);
            r&&(g&&(r.className+=" jwcapBottom"),a.appendChild(r));
            b(B(".jw"+f+" .jwrail"),{
                left:g?y:A.width,
                right:g?y:n.width,
                top:g?A.height:y,
            ...
            K[f]=a;
            a.vertical=g;
            "time"==f?(ja=new h.overlay(Z+"_timetooltip",ba),Ia=new h.thumbs(Z+"_thumb"),Sa=z.createElement("div"),Sa.className="jwoverlaytext",Ja=z.createElement("div"),f=Ia.element(),Ja.appendChild(f),Ja.appendChild(Sa),ja.setContents(Ja),Va=d,W(0),f=ja.element(),d.appendChild(f),K.timeSliderThumb&&b(B(".jwtimeSliderThumb"),{
                "margin-left":H("timeSliderThumb").width/-2
                }),fa(0),T(0),T(0),fa(0)):"volume"==f&&(b(B(".jwvolume"),{
...
function V(a){
    for(var b in ta)b!=a&&ta.hasOwnProperty(b)&&ta[b].hide()
        }
        function N(){
    za||(Aa.show(),V("volume"))
    }
    ...
    function Ea(a){
    0<=a&&a<la.length&&(G.jwSetCurrentQuality(a),ga.hide())
    }
    function Na(a){
    0<=a&&a<ha.length&&(G.jwSetCurrentCaptions(a),ia.hide())
    }
    function Oa(a){
    a.preventDefault();
...
function pa(b){
    return function(c){
        0==
        c.button&&(K[b+"Rail"].className="jwrail","time"==b?G.jwGetState()!=a.IDLE&&(G.jwSeekDrag(n),qa=b):qa=b)
        }
    }
function ua(a){
    ...
    if(50<b-Xa){
        if((Pa=f.bounds(Va))&&0!=Pa.width){
            var c=ja.element(),d=a.pageX-Pa.left-p.pageXOffset;
            0<=d&&d<=Pa.width&&(c.style.left=Math.round(d)+"px",W(na*d/Pa.width),va=f.bounds(S))
            }
            Xa=b
        }
        if(qa&&0==a.button)return c=K[qa].getElementsByClassName("jwrail")[0],d=f.bounds(c),c=qa,d=K[c].vertical?(d.bottom-a.pageY)/d.height:(a.pageX-d.left)/d.width,"mouseup"==
        a.type?("time"==c&&G.jwSeekDrag(q),K[c+"Rail"].className="jwrail jwsmooth",qa=t,Ya[c](d)):("time"==qa?T(d):Q(d),500<b-Za&&(Za=b,Ya[qa](d))),!1
        }
        function R(){
    ...
    Ba[a]=b;
    if(Y[a]&&(b=Y[a],a=Ba[a],b&&0<b.elements.length))for(var c=0;c<b.elements.length;c++){
    ...
    function Qa(){
    1<G.jwGetPlaylist().length&&(!z.querySelector("#"+G.id+" .jwplaylist")||G.jwGetFullscreen())?(b(B(".jwnext"),r),b(B(".jwprev"),
        r),b(B(".nextdiv"),r)):(b(B(".jwnext"),j),b(B(".jwprev"),j),b(B(".nextdiv"),j))
    }
    function X(a){
    va||(va=f.bounds(S));
    a.offsetX(0);
    ...
    function Ca(a,b){
    var c=K[a].vertical,d=a+("time"==a?"Slider":""),e=100*Math.min(Math.max(0,b),1)+"%",f=K[d+"Progress"],
    d=K[d+"Thumb"],g=q;
...
K.fullscreen&&(Ka=new h.overlay(Z+"_fullscreenoverlay",ba),Ga=z.createElement("div"),Ga.className="jwoverlaytext",Ga.innerHTML="Fullscreen",Ka.setContents(Ga),xa(Ka,K.fullscreen,ca),ta.fullscreen=Ka);
b(B(".jwright"),{
    right:H("capRight").width
    });
aa&&S.appendChild(aa);
...
G.jwAddEventListener(d.JWPLAYER_PLAYER_STATE,function(c){
    switch(c.newstate){
        case a.BUFFERING:case a.PLAYING:
...
G.jwAddEventListener(d.JWPLAYER_PLAYLIST_ITEM,function(a){
    ...
    if("array"==f.typeOf(a))for(var b=0;b<a.length;b++)if(a[b].file&&a[b].kind&&"thumbnails"==a[b].kind.toLowerCase()){
        Ia.load(a[b].file);
        return
    }
    Ia.load()
...
G.jwAddEventListener(d.JWPLAYER_MEDIA_LEVELS,function(a){
    if((la=a.levels)&&1<la.length&&ga){
        ...
        ga.clearOptions();
        for(var c=0;c<la.length;c++)ga.addOption(la[c].label,
            c);
        A(a)
        }else b(B(".jwhd"),{
        display:"none"
...
G.jwAddEventListener(d.JWPLAYER_CAPTIONS_LIST,function(a){
    if((ha=a.tracks)&&1<ha.length&&ia){
        ...
        ia.clearOptions();
        for(var c=0;c<ha.length;c++)ia.addOption(ha[c].label,c);
        E(a)
        }else b(B(".jwcc"),{
        display:"none"
    });
...
(function(g){
    var h=g.html5,f=g.utils,d=g.events,a=d.state;
    h.controller=function(b,c){
        ...
        try{
            0<=u&&(e(u),u=-1);
            v=m;
            if(!x&&(x=!0,w.sendEvent(d.JWPLAYER_MEDIA_BEFOREPLAY),x=!1,D)){
                D=!1;
                v=null;
                return
            }
            if(p.state==a.IDLE){
                if(0==p.playlist.length)return!1;
                z.load(p.playlist[p.item])
                }else p.state==a.PAUSED&&z.play();
            return!0
            }catch(c){
            w.sendEvent(d.JWPLAYER_ERROR,c),v=null
            }
            return!1
        }
        function l(b){
        v=null;
        try{
            return p.state!=a.IDLE?z.stop():b||(C=!0),x&&(D=!0),!0
            }catch(c){
        ...
        try{
            switch(p.state){
                case a.PLAYING:case a.BUFFERING:
                    z.pause();
                    break;
                default:
                    x&&(D=!0)
    ...
    var p=b,z=b.getVideo(),w=new d.eventdispatcher(p.id,
    p.config.debug),s=!1,u=-1,x,v,C=!1,D,A=[];
f.extend(this,w);
    this.play=t(m);
    this.pause=t(j);
...
this.detachMedia=function(){
    ...
    this.attachMedia=function(a){
        try{
            p.getVideo().attachMedia(a),"function"==typeof v&&v()
            }catch(b){
            return null
            }
        };
...
function(a){
    if(!s){
        ...
        for(s=!0;0<A.length;)a=A.shift(),y(a.method,a.arguments)
            }
        };
...
p.addEventListener(d.JWPLAYER_MEDIA_COMPLETE,
    function(){
        setTimeout(n,25)
        });
...
(function(g){
    var h=jwplayer.utils,f=jwplayer.events,d=f.state,a=h.css,b=document,c=".jwpreview",k=!0,e=!1;
    g.display=function(m,l){
        function j(){
            switch(s.jwGetState()){
                case d.PLAYING:case d.BUFFERING:
                    s.jwPause();
                    break;
                default:
                    s.jwPlay()
            ...
            if(a!=U)switch(U=a,F&&F.setRotation(0),a){
                case d.IDLE:
    ...
    s.jwAddEventListener(f.JWPLAYER_MEDIA_ERROR,
        p);
...
(function(g){
    var h=jwplayer.utils,f=h.css,d=void 0,a=document,b="none",c="100%";
    ...
    this.setText=function(b){
        ...
        c.height="0";
        c.display="block";
        if(b)for(;2<Math.floor(C.scrollHeight/a.defaultView.getComputedStyle(C,null).lineHeight.replace("px",""));)C.innerHTML=C.innerHTML.replace(/(.*) .*$/,"$1...");
        c.height="";
        c.display="";
        t()
        };
...
(function(g){
    var h=jwplayer.utils,f=h.css,d=h.bounds,a=".jwdockbuttons",b=document,c="none",k="block";
    g.dock=function(e,m){
        function l(a,b){
            var c=q(a);
            f(j("."+a),{
    ...
    v.addButton=function(a,b,c,e){
        if(!w[e]){
            var h=r("div","divider",
                x),k=r("button",null,x),j=r("div",null,k);
            j.id=y+"_"+e;
            j.innerHTML="\x26nbsp;";
            f("#"+j.id,{
                "background-image":a
...
(function(g){
    var h=jwplayer,f=h.utils,d=h.events,a=d.state,b=h.playlist;
    g.instream=function(c,h,e,m){
        ...
        this.load=function(e,F){
            ...
            w=m.detachMedia();
            v||(v=new g.video(w),v.addGlobalListener(l),v.addEventListener(d.JWPLAYER_MEDIA_META,q),v.addEventListener(d.JWPLAYER_MEDIA_COMPLETE,r),v.addEventListener(d.JWPLAYER_MEDIA_BUFFER_FULL,j));
            v.attachMedia();
            v.mute(h.mute);
            v.volume(h.volume);
            B=new g.model({},v);
        ...
        this.jwInstreamDestroy=function(b){
            if(A){
                ...
                m.attachMedia();
                if(u==a.BUFFERING||u==a.PLAYING)w.play(),h.playlist[h.item]==x&&h.getVideo().seek(s)
                    }
                };
    ...
    this.jwStop=function(){
        "true"==
        p.controlbarstoppable.toString().toLowerCase()&&(this.jwInstreamDestroy(),c.jwStop())
        };
...
(function(g){
    var h=g.utils,f=h.css,d=g.events.state,a=g.html5.logo=function(b,c){
        function k(a){
            h.exists(a)&&a.stopPropagation();
            if(!q||!l.link)e.jwGetState()==d.IDLE||e.jwGetState()==d.PAUSED?e.jwPlay():e.jwPause();
            q&&l.link&&(e.jwPause(),e.jwSetFullscreen(!1),window.open(l.link,l.linktarget))
            }
        ...
        this.hide=function(a){
            if(l.hide||a)q=!1,j.style.visibility="hidden",j.style.opacity=0
                };
        ...
        if("o"==n||"f"==n)r.link="http://www.longtailvideo.com/jwpabout/?a\x3dl\x26v\x3d"+g.version+"\x26m\x3dh\x26e\x3d"+n;
        l=h.extend({},r,c);
        ...
        j.id=m;
        if(l.file){
            var r=/(\w+)-(\w+)/.exec(l.position),n={},t=l.margin;
...
(function(g){
    var h=g.html5,f=g.utils,d=f.css,a=void 0;
    ...
    var r=e,q=new h.overlay(c+"_overlay",g);
    e=f.extend({
    fontcase:a,
    fontcolor:"#cccccc",
    fontsize:11,
    ...
    this.clearOptions=function(){
        for(;0<t.length;)n.removeChild(t.pop())
            };
...
n=l("jwmenu");
    ...
    b);
b=j("menuOption");
    var z=j("menuOptionOver"),w=j("menuOptionActive");
...
(function(g){
    var h=jwplayer.utils,f=jwplayer.events;
    g.model=function(d,a){
        function b(a){
            var b=r[a.type]?r[a.type].split(","):[],d,e;
            if(0<b.length){
                for(d=0;d<b.length;d++){
                    var f=b[d].split("-\x3e"),g=f[0],f=f[1]?f[1]:g;
                    c[f]!=a[g]&&(c[f]=a[g],e=!0)
                    }
    ...
    c.setFullscreen=function(a){
        a!=c.fullscreen&&(c.fullscreen=a,c.sendEvent(f.JWPLAYER_FULLSCREEN,{
            fullscreen:a
        }))
        };
...
c.setItem=function(a){
    ...
    1:a;
    a!=c.item&&(c.item=a,c.sendEvent(f.JWPLAYER_PLAYLIST_ITEM,{
        index:c.item
        }))
    };
...
h.extend(c,new f.eventdispatcher);
    ...
    for(q in m)m[q]=h.serialize(m[q]);e.config=
    m;
    h.extend(c,{
        id:d.id,
        state:f.state.IDLE,
...
(function(g){
    var h=g.utils,f=h.css,d=h.transitionStyle,a="top",b="bottom",c="right",k="left",e=void 0,m=document,l={
        fontcase:e,
        fontcolor:"#ffffff",
        fontsize:12,
        fontweight:e,
    ...
    g.html5.overlay=function(d,g,q){
        function n(a){
            ...
            function p(d,g){
            g||(g="");
            var h=y("cap"+d+g,"jwborder jw"+d+(g?g:"")),j=h[0],h=h[1],l={
            ...
            g&&(j[g]=h[g],j[d]=0,l[d]=h[d],l[g]=0,f(n("jw"+d),j),f(n("jw"+g),l),E[d]=h[d],E[g]=h[g])
            }
            function z(){
            0!=
            u.clientWidth&&(f(n(),{
...
(function(g){
    ...
    g.player=function(f){
        function d(a){
            ...
            for(c in a)b[c]=a[c];b.sources=[];
            b.tracks=[];
            if(0<a.sources.length)for(c in a.sources){
                var d=a.sources[c];
                b.sources.push({
                    file:d.file,
                ...
                if(0<a.tracks.length)for(c in a.tracks)d=a.tracks[c],b.tracks.push({
                file:d.file,
...
(function(g){
    var h={
        ...
        fontsize:11,
        titlesize:13
    },f=jwplayer.events,d=jwplayer.utils,a=d.css,b=document;
    ...
    n.appendChild(t);
    for(var v in x)x[v]=r.getSkinElement("playlist",
        v);x.item&&(u=x.item.height);
    var C=v=r=0;
    d.clearCss(e());
    a(e(),{
    ...
    j.jwAddEventListener(f.JWPLAYER_PLAYLIST_LOADED,function(){
        t.innerHTML="";
        for(var b=j.jwGetPlaylist(),c=[],e=0;e<b.length;e++)b[e]["ova.hidden"]||c.push(b[e]);
        if(y=c){
            b=m("ul","jwlist");
            b.id=n.id+"_ul"+Math.round(1E7*Math.random());
            p=b;
            for(b=0;b<y.length;b++){
                var f=b,c=y[f],e=m("li","jwitem"),h=void 0;
                e.id=p.id+"_item_"+f;
                0<f&&(h=m("div","jwplaylistdivider"),e.appendChild(h));
                f=m("div","jwplaylistimg jwfill");
                if(c.image||c["playlist.image"]||x.itemImage)h=
                    void 0,c["playlist.image"]?h=c["playlist.image"]:c.image?h=c.image:x.itemImage&&(h=x.itemImage.src),a("#"+e.id+" .jwplaylistimg",{
                ...
                f.appendChild(h);
                c.description&&(h=m("span","jwdescription"),h.innerHTML=c.description,f.appendChild(h));
                e.appendChild(f);
                c=e;
                c.onclick=l(b);
                p.appendChild(c)
...
j.jwAddEventListener(f.JWPLAYER_PLAYLIST_ITEM,function(a){
    0<=z&&(b.getElementById(p.id+"_item_"+z).className="jwitem",z=a.index);
    b.getElementById(p.id+"_item_"+a.index).className="jwitem active";
    a=j.jwGetPlaylistIndex();
    a!=w&&(w=-1,d.isIOS()&&window.iScroll?p.scrollTop=a*u:s&&s.visible()&&s.thumbPosition(a/(j.jwGetPlaylist().length-1)))
    });
return this
};
...
(function(g){
    var h=jwplayer,f=h.utils,d=h.events;
    g.playlistloader=function(){
        function a(a){
            try{
                var b=a.responseXML.firstChild;
                "xml"==g.parsers.localName(b)&&(b=b.nextSibling);
                if("rss"!=g.parsers.localName(b))c("Not a valid RSS feed");
                else{
                    var l=new h.playlist(g.parsers.rssparser.parse(b));
                    if(a=l){
                        var b=[],j,r,q;
                        for(j=0;j<a.length;j++)if(r=a[j],(q=f.filterSources(r.sources))&&q.length)r.sources=q,b.push(r);l=b
                        }else l=void 0;
...
(function(g){
    function h(){
        ...
        var f=jwplayer.utils,d=f.css,a=document,b=window,c=void 0;
    g.playlistslider=function(g,e,h,l){
        ...
        function n(a){
        if(A)return a=a?a:b.event,N(v-(a.detail?-1*a.detail:a.wheelDelta/40)/10),a.stopPropagation&&a.stopPropagation(),a.preventDefault&&a.preventDefault(),a.cancelBubble=!0,a.cancel=!0,a.returnValue=!1
            }
            function t(a){
        if(x||"click"==a.type){
            var b=f.bounds(s),c=u.clientHeight/2;
...
(function(g){
    var h=jwplayer.utils,f=h.css,d=document,a="none";
    g.rightclick=function(b,c){
        ...
        m.oncontextmenu=function(b){
            if(!j){
                ...
                var c=h.bounds(m),d=c.top,c=c.left;
                r.style.display=a;
...
(function(g){
    var h=jwplayer,f=h.utils,d=h.events,a=h.playlist,b=2,c=3,k=4;
        ...
        function l(a,b,c){
            ...
            for(var a=0;a<D.length;a++){
                var b=D[a],c;
                    ...
                    if(c=b.depends){
                        c=c.toString().split(",");
                        for(var d=0;d<c.length;d++)if(!u[c[d]]){
                            c=!1;
                            break a
                        }
                        }
...
(function(g){
    var h=jwplayer.utils,f="Skin formatting error";
    g.skinloader=function(d,a,b){
        ...
        if(0===a.length)q(r);else for(b=0;b<a.length;b++){
            var c=
            j(a[b].getAttribute("name")),d={
                settings:{},
                elements:{},
                layout:{}
            ...
            for(e=0;e<c.length;e++){
                var f=c[e].getAttribute("name"),g=c[e].getAttribute("value");
...
function m(){
    ...
    z||(y=setInterval(function(){
        a:{
            for(var a in r)if("properties"!=a)for(var b in r[a].elements)if(!(r[j(a)]?r[j(a)].elements[j(b)]:null).ready)break a;
...
function l(a,b){
    b=j(b);
    var c=new Image,d=j(a.getAttribute("name")),e=a.getAttribute("src");
    if(0!==e.indexOf("data:image/png;base64,"))var f=h.getAbsolutePath(p),e=[f.substr(0,f.lastIndexOf("/")),b,e].join("/");
    r[b].elements[d]={
        height:0,
        width:0,
        src:"",
    ...
    c.onload=function(){
        var a=b,e=r[j(a)]?r[j(a)].elements[j(d)]:null;
...
(function(g){
    var h=jwplayer.utils,f=jwplayer.events,d=h.css;
    g.thumbs=function(a){
        function b(a){
        ...
        this.updateTimeline=function(a){
            var b=0;
            if(m){
                for(;b<m.length&&a>m[b].end;)b++;
                b==m.length&&b--;
                if(m[b].text)if(a=m[b].text,0>a.indexOf("://")&&(a=l?l+"/"+a:a),0<a.indexOf("#xywh"))try{
...
(function(g){
    var h=g.utils,f=g.events,d=f.state;
    g.html5.video=function(a){
        function b(a,b){
            ...
            function c(){}
        function g(){
            F&&(E==d.PLAYING&&!A)&&(x=Number(s.currentTime.toFixed(1)),b(f.JWPLAYER_MEDIA_TIME,{
                position:x,
                duration:u
            }))
            }
            function e(a){
            F&&(v||(v=!0,C||(C=!0,b(f.JWPLAYER_MEDIA_BUFFER_FULL))),"loadedmetadata"==a.type&&(s.muted&&(s.muted=!1,s.muted=!0),b(f.JWPLAYER_MEDIA_META,{
                duration:s.duration,
        ...
        s.load();
        B=setInterval(n,100);
        if((h.isIPod()||h.isAndroid(2.3))&&!C)C=!0,b(f.JWPLAYER_MEDIA_BUFFER_FULL)
            }
            function q(a){
    ...
    if(F){
        ...
        a!=J&&(J=a,b(f.JWPLAYER_MEDIA_BUFFER,{
...
function t(){
    E!=d.IDLE&&(U=-1,b(f.JWPLAYER_MEDIA_BEFORECOMPLETE),ca=!0,F&&(q(d.IDLE),b(f.JWPLAYER_MEDIA_COMPLETE),ca=!1))
    }
    var y=h.isIE(),p={
    abort:c,
    canplay:e,
...
error:function(){
    F&&(h.log("Error playing media: %o",s.error),O.sendEvent(f.JWPLAYER_MEDIA_ERROR,{
...
N.load=function(a){
    if(F){
        ...
        M=z.sources;
        0>U&&(U=0);
        for(a=0;a<M.length;a++)if(M[a]["default"]){
            U=a;
            break
        }
        var b=h.getCookies().qualityLabel;
        if(b)for(a=
            0;a<M.length;a++)if(M[a].label==b){
            U=a;
            break
...
N.attachMedia=function(a){
    F=!0;
    a||(v=!1);
    ca&&(q(d.IDLE),b(f.JWPLAYER_MEDIA_COMPLETE),ca=!1)
    };
...
N.setCurrentQuality=function(a){
    U!=a&&(a=parseInt(a),0<=a&&(M&&M.length>a)&&(U=a,h.saveCookie("qualityLabel",M[a].label),b(f.JWPLAYER_MEDIA_LEVEL_CHANGED,
...
s=a;
for(var Ea in p)s.addEventListener(Ea,p[Ea],!1);s.controls=!0;
s.controls=!1;
...
(function(g){
    var h=jwplayer.utils,f=jwplayer.events,d=f.state,a=h.css,b=h.isMobile(),c=h.isIPad(),k=h.isIPod(),e=h.isAndroid(),m=h.isIOS(),l=document,j="jwmain",r="jwvideo",q="jwplaylistcontainer",n=!0,t=!1,y="hidden",p="none",z="block";
    g.view=function(w,s){
        function u(a){
            ...
            function v(){
            clearTimeout(ma);
            if(R.jwGetState()==d.PLAYING||R.jwGetState()==
                d.PAUSED)$(),na||(ma=setTimeout(A,Qa))
                }
                function C(){
            clearTimeout(ma);
            na=n
            ...
            function A(){
            R.jwGetState()!=d.BUFFERING&&(Q&&(!P&&!Y)&&Q.hide(),H&&(!oa&&!Y)&&H.hide(),V());
            clearTimeout(ma);
            ma=0
            }
            function E(a){
            ...
            var d=L.playlistsize,e=L.playlistposition;
            ...
            Q&&(P?(Q.audioMode(n),$(),T.hidePreview(n),ca(),ka(t)):(Q.audioMode(t),Oa(R.jwGetState())));
            G&&P&&V();
            ...
            function F(a){
        m||(a?(W.className+=" jwfullscreen",l.getElementsByTagName("body")[0].style["overflow-y"]=y):(W.className=W.className.replace(/\s+jwfullscreen/,""),l.getElementsByTagName("body")[0].style["overflow-y"]=""))
        }
        function M(){
        var a;
            a:{
            a=[l.mozFullScreenElement,l.webkitCurrentFullScreenElement,X.webkitDisplayingFullscreen];
            for(var b=0;b<a.length;b++)if(a[b]&&(!a[b].id||a[b].id==R.id)){
                a=n;
                break a
            }
            a=t
        ...
        if(b&&!Y&&(e&&(da.style.display=z),!b||!L.fullscreen))X.controls=!1
            }
            function ca(){
        T&&(b&&!Y&&(e&&L.controls&&(da.style.display=p),X.controls=L.controls),T.hide())
        }
        function $(){
        if(L.controls||P)Q&&Q.show(),U();
        G&&!P&&G.show()
        }
        function ka(b){
        b=b&&!P;
...
this.setup=function(a){
    if(!K){
        ...
        xa=x("span",q);
        a=L.height;
        var c=L.componentConfig("controlbar"),e=L.componentConfig("display");
        B(a);
        ...
        if(!b||Y)Q=new g.controlbar(R,c),da.appendChild(Q.element()),Y&&$();
        setTimeout(function(){
            I(L.width,L.height)
            },0);
        wa.appendChild(fa);
...
var la=this.fullscreen=function(a){
    ...
    a?L.fullscreen||(F(n),W.requestFullScreen?W.requestFullScreen():W.mozRequestFullScreen?W.mozRequestFullScreen():W.webkitRequestFullScreen&&W.webkitRequestFullScreen(),L.setFullscreen(n)):(F(t),L.fullscreen&&(L.setFullscreen(t),l.cancelFullScreen?
        l.cancelFullScreen():l.mozCancelFullScreen?l.mozCancelFullScreen():l.webkitCancelFullScreen?l.webkitCancelFullScreen():X.webkitExitFullScreen&&X.webkitExitFullScreen()),c&&R.jwGetState()==d.PAUSED&&setTimeout(N,500));
    Q&&Q.redraw();
    T&&T.redraw();
    H&&H.redraw();
    J();
...
this.setControls=function(a){
    ...
    a?n:t;
    L.controls=a;
    a!=b&&(a?N():(Q&&(!P&&!Y)&&Q.hide(),H&&(!oa&&!Y)&&H.hide(),V(),ca()),ra.sendEvent(f.JWPLAYER_CONTROLS,{
        controls:a
    }))
    };
...
this.getSafeRegion=function(){
    var a=L.controls,b=h.bounds(wa),c=b.top,d=h.bounds(Q?Q.element():null),e=0<H.numButtons(),f=h.bounds(H.element()),g=h.bounds(G.element()),j=0==G.position().indexOf("top"),e=Math.max(e?f.top+f.height-c:0,j?g.top+g.height-c:0),f=b.width,
...
(function(g){
    ...
    g.model=function(f,d){
        var a=new jwplayer.utils.key(f.key),b=new h(f,d),c=b.componentConfig;
        b.edition=function(){
            return a.edition()
            };
...
(function(g){
    var h=jwplayer.utils.extend,f=g.rightclick;
    g.rightclick=function(d,a){
        if("free"==d.edition())a.aboutlink="http://www.longtailvideo.com/jwpabout/?a\x3dr\x26v\x3d"+g.version+"\x26m\x3dh\x26e\x3df",delete a.abouttext;
        else{
            if(!a.aboutlink){
                var b="http://www.longtailvideo.com/jwpabout/?a\x3dr\x26v\x3d"+g.version+"\x26m\x3dh\x26e\x3d",c=d.edition();
                ...
                b.substr(1),a.abouttext="About JW Player "+g.version+" ("+b+" edition)")
            }
            h(this,new f(d,a))
        }
    })(jwplayer.html5);
