...
b.getAbsolutePath=function(a,e){
    ...
    if(b.exists(a)){
        ...
        var g=e.substring(c.length,e.indexOf("/",c.length+1)),k;
...
b.translateEventResponse=function(a,e){
    var c=b.extend({},e);
    a==d.events.JWPLAYER_FULLSCREEN&&!c.fullscreen?(c.fullscreen="true"==c.message?!0:!1,delete c.message):"object"==typeof c.data?(c=b.extend(c,c.data),delete c.data):"object"==typeof c.metadata&&b.deepReplaceKeyName(c.metadata,["__dot__","__spc__","__dsh__","__default__"],
        ["."," ","-","default"]);
    var k=["position","duration","offset"],g;
    for(g in k)c[k[g]]&&(c[k[g]]=Math.round(1E3*c[k[g]])/1E3);return c
    };
...
b.deepReplaceKeyName=function(b,a,e){
    switch(d.utils.typeOf(b)){
        ...
        case "object":
            for(var k in b){
            var f;
            if(a instanceof Array&&e instanceof Array)if(a.length!=e.length)continue;else f=a;else f=[a];
            for(var g=k,c=0;c<f.length;c++)g=g.replace(RegExp(a[c],
                "g"),e[c]);
...
b.repo=function(){
    ...
    for(b in g)c[b]={
        ...
        for(b in a)c[b]||(c[b]={}),c[b].flash=a[b];c.types=h;
    c.mimeType=function(a){
        for(var b in h)if(h[b]==
            a)return b
            };
    ...
    d.scriptloader=function(g){
        function c(){
            ...
            var e=a.NEW,k=jwplayer.events,l=new k.eventdispatcher;
        d.extend(this,l);
        this.load=function(){
            var l=d.scriptloader.loaders[g];
            if(l&&(l.getStatus()==a.NEW||l.getStatus()==a.LOADING))l.addEventListener(k.ERROR,c),l.addEventListener(k.COMPLETE,
                b);
...
d.scriptloader.loaders={}
    ...
    d.xmlAttribute=function(a,d){
        for(var g=0;g<a.attributes.length;g++)if(a.attributes[g].name&&a.attributes[g].name.toLowerCase()==d.toLowerCase())return a.attributes[g].value.toString();return""
        };
    ...
    a.encrypt=function(c,b){
        ...
        for(var k=a.strToLongs(g.encode(b).slice(0,16)),l=e.length,d=e[l-1],n=e[0],q,j=Math.floor(6+52/l),f=0;0<j--;){
            f+=2654435769;
            q=f>>>2&3;
            for(var r=0;r<l;r++)n=e[(r+1)%l],d=(d>>>5^n<<2)+(n>>>3^d<<4)^(f^n)+(k[r&3^q]^d),d=e[r]+=d
                }
                e=a.longsToStr(e);
        return h.encode(e)
        };
    ...
    a.decrypt=function(c,b){
        ...
        for(var e=a.strToLongs(h.decode(c)),k=a.strToLongs(g.encode(b).slice(0,16)),d=e.length,
            ...
            q=j>>>2&3;
            for(var f=d-1;0<=f;f--)m=e[0<f?f-1:d-1],m=(m>>>5^n<<2)+(n>>>3^m<<4)^(j^n)+(k[f&3^q]^m),n=e[f]-=m;
            j-=2654435769
            }
            e=a.longsToStr(e);
        e=e.replace(/\0+$/,"");
    ...
    var h={
        ...
        encode:function(a,b){
            var e,k,d,m,n=[],q="",j,f,r=h.code;
            f=("undefined"==typeof b?0:b)?g.encode(a):a;
            j=f.length%3;
            if(0<j)for(;3>j++;)q+="\x3d",f+="\x00";
            for(j=0;j<f.length;j+=3)e=f.charCodeAt(j),k=f.charCodeAt(j+1),d=f.charCodeAt(j+2),m=e<<16|k<<8|d,e=m>>18&63,k=m>>12&63,d=m>>6&63,m&=63,n[j/3]=r.charAt(e)+r.charAt(k)+r.charAt(d)+
                r.charAt(m);
            n=n.join("");
        ...
        decode:function(a,b){
            ...
            var e,k,d,m,n,q=[],j,f=h.code;
            j=b?g.decode(a):a;
            for(var r=0;r<j.length;r+=4)e=f.indexOf(j.charAt(r)),k=f.indexOf(j.charAt(r+1)),m=f.indexOf(j.charAt(r+2)),n=f.indexOf(j.charAt(r+3)),d=e<<18|k<<12|m<<6|n,e=d>>>16&255,k=d>>>8&255,d&=255,q[r/4]=String.fromCharCode(e,k,d),64==n&&(q[r/4]=String.fromCharCode(e,k)),64==m&&(q[r/4]=String.fromCharCode(e));
            m=q.join("");
...
g={
    ...
    d.eventdispatcher=function(d,g){
        ...
        this.addEventListener=function(b,k,d){
            try{
                a.exists(c[b])||(c[b]=[]),"string"==a.typeOf(k)&&(k=(new Function("return "+k))()),c[b].push({
                    listener:k,
                    count:d
                })
                }catch(g){
        ...
        this.removeEventListener=function(b,d){
            if(c[b]){
                try{
                    for(var g=0;g<c[b].length;g++)if(c[b][g].listener.toString()==
                        d.toString()){
                        c[b].splice(g,1);
                        break
                    }
...
function(d){
    var a=jwplayer.utils,h=jwplayer.events;
    d.pluginmodes={
        FLASH:0,
        JAVASCRIPT:1,
        HYBRID:2
    ...
    d.plugin=function(g){
        ...
        var k=a.loaderstatus.NEW,l,m,n,q,j=new h.eventdispatcher;
    a.extend(this,
        j);
    this.load=function(){
        if(k==a.loaderstatus.NEW)if(0<g.lastIndexOf(".swf"))l=g,k=a.loaderstatus.COMPLETE,j.sendEvent(h.COMPLETE);
            else if(a.getPluginPathType(g)==a.pluginPathType.CDN)k=a.loaderstatus.COMPLETE,j.sendEvent(h.COMPLETE);
            else{
...
this.getPluginmode=function(){
    if("undefined"!=
        typeof l&&"undefined"!=typeof m)return d.pluginmodes.HYBRID;
    ...
    d.plugins.pluginloader=function(g,c){
        function b(){
            ...
            if(!l&&!m){
                ...
                c=g.getPlugins(),f;
                for(f in q){
                    var k=a.getPluginName(f),h=c[k],k=h.getJS(),j=h.getTarget(),h=h.getStatus();
                    if(h==a.loaderstatus.LOADING||h==a.loaderstatus.NEW)e++;
                    else if(k&&(!j||parseFloat(j)>parseFloat(d.version)))m=!0,n="Incompatible player version",b()
                        }
                        0==e&&b()
                }
            }
        var k=a.loaderstatus.NEW,l=!1,m=!1,n,q=c,j=new h.eventdispatcher;
    a.extend(this,j);
    this.setupPlugins=function(b,e,c){
        var f={
            length:0,
            plugins:{}
    },d=0,k={},h=g.getPlugins(),l;
    for(l in e.plugins){
        var j=a.getPluginName(l),m=h[j],B=m.getFlashPath(),
        n=m.getJS(),q=m.getURL();
...
this.getStatus=function(){
    ...
    var a=
    d.item=function(h){
        var g=jwplayer.utils,c=g.extend({},a.defaults,h);
    ...
    d.source=function(d){
        var c=a.extend({},h),b;
        for(b in h)a.exists(d[b])&&(c[b]=d[b],delete d[b]);c.type&&0<c.type.indexOf("/")&&(c.type=a.extensionmap.mimeType(c.type));
    ...
    d.track=function(d){
        var c=a.extend({},h);
        d||(d={});
        for(var b in h)a.exists(d[b])&&(c[b]=d[b],delete d[b]);return c
        }
    }(jwplayer.playlist),function(d){
    var a=d.utils,h=d.events,g=document,c=d.embed=function(b){
        function e(a){
            ...
            if(m.fallback){
                ...
                c.opacity=1;
                var c=
                document.createElement("p"),d=c.style;
        ...
        var m=new c.config(b.config),n,q,j,f=m.width,r=m.height,p="Error loading player: ",s=d.plugins.loadPlugins(b.id,m.plugins);
    ...
    s.addEventListener(h.COMPLETE,function(){
        if("array"==a.typeOf(m.playlist)&&2>m.playlist.length&&(0==m.playlist.length||!m.playlist[0].sources||0==m.playlist[0].sources.length))k();
            ...
            for(var d=0;d<m.modes.length;d++)if(m.modes[d].type&&c[m.modes[d].type]){
                var f=a.extend({},m),g=new c[m.modes[d].type](n,m.modes[d],f,s,b);
                if(g.supportsConfig()){
                    ...
                    f=f.events;
                    g=void 0;
                    for(g in f)"function"==typeof d[g]&&d[g].call(d,f[g]);return b
                    }
                }
...
return b
    ...
    function a(a){
        if(a.playlist)for(var e=0;e<a.playlist.length;e++)a.playlist[e]=new c(a.playlist[e]);
            ...
            var e={},d;
            for(d in c.defaults)h(a,e,d);e.sources||(a.levels?(e.sources=a.levels,delete a.levels):(d={},h(a,d,"file"),h(a,d,"type"),e.sources=d.file?[d]:[]));
            a.playlist=[new c(e)]
            }
        }
    function h(a,e,c){
...
(d.embed.config=function(b){
    ...
    d.embed.download=function(d,c,b){
        function e(a,b){
            ...
            var l=a.extend({},c),m,n=l.width?l.width:480,q=l.height?l.height:320,j;
        c=c.logo?c.logo:{
            prefix:a.repo(),
            file:"logo.png",
            margin:10
        ...
        if(r&&r.length){
            ...
            for(r=0;r<s.length;r++){
                var u=s[r],t=u.type?u.type:a.extensionmap.extType(a.extension(u.file));
    ...
    function q(b){
        ...
        var j=new d.events.eventdispatcher,f=a.flashVersion();
    a.extend(this,j);
    this.embed=function(){
        e.id=l.id;
        ...
        var s=a.getCookies(),t;
        for(t in s)"undefined"==typeof p[t]&&(p[t]=s[t]);t=window.location.pathname.split("/");
...
return!1
    ...
    var q=this,j=new g.eventdispatcher;
a.extend(q,j);
q.embed=function(){
    if(d.html5){
        ...
        l.setPlayer(f,"html5")
        }else f=new a.scriptloader(b.src),f.addEventListener(g.ERROR,n),f.addEventListener(g.COMPLETE,q.embed),f.load()
        };
...
q.supportsConfig=function(){
    ...
    var a=d.embed,h=d.utils,g=h.extend(function(c){
        var b=h.repo(),e=c.config,g=e.plugins,l=e.analytics,m=b+"jwpsrv.js",n=b+"sharing.js",q=b+"related.js",j=b+"gapro.js",f=(new d.utils.key(d.key)).edition(),g=g?g:{};
        ...
        delete g.sharing;
        delete g.related;
        if("premium"==f||"ads"==f)e.sharing&&(e.sharing.client&&e.sharing.client.match(".js$|.swf$")&&(n=e.sharing.client),g[n]=e.sharing),e.related&&(e.related.client&&e.related.client.match(".js$|.swf$")&&(q=e.related.client),g[q]=
...
function q(a,b){
    ...
    var f=this,r={},p={},s=void 0,u=!1,t=[],w=void 0,x={},y={};
...
f.addButton=function(a,b,c,e){
    try{
        y[e]=c,j("jwDockAddButton",a,b,"jwplayer('"+f.id+"').callback('"+e+
            "')",e)
        }catch(d){
...
f.pause=function(a){
    "undefined"==
    typeof a?(a=f.getState(),a==c.PLAYING||a==c.BUFFERING?j("jwPause"):j("jwPlay")):j("jwPause",a);
    return f
    };
...
f.dispatchEvent=function(a,b){
    if(r[a])for(var c=h.translateEventResponse(a,b),e=0;e<r[a].length;e++)if("function"==
        typeof r[a][e])try{
        r[a][e].call(this,c)
        }catch(d){
...
e.destroyPlayer=function(c){
    for(var e=
        -1,d,g=0;g<a.length;g++)a[g].id==c&&(e=g,d=a[g]);
